#!/usr/bin/env python3
"""
AC Semiotic Lexical Framework (SLF) Transform Modual Interactive v.0.0.3
...
def show_status(engine):
    status = (
        f"[bold magenta]Seed:[/bold magenta] [white]{engine.base_seed}[/white]\n"
        ...
        f"[bold green]Branch Limb:[/bold green] [white]{engine.step}[/white]\n"
    )
...
class TransformEngine:
    ...
    def _startup(self, metadata_paths: list[str], seed: str, author: str = None):
        ...
        self.base_seed = self._normalize(initial_seed)
        self.working_seed = self.base_seed
